...
def check_user_directories(root_dir, user_name):
	...
	if not os.path.exists(os.path.join(root_dir, user_name)):
		print(ERR_USER_DIR_NOT_EXISTS % user_name)
		return INVALID

	files_path = generate_user_paths(root_dir, user_name)
	if not os.path.exists(files_path):
		print(ERR_USER_FILE_DIR_NOT_EXISTS % user_name)
		return INVALID
...
def send_data(conn, logger_adapter, response_code, data):
	try:
		...
		# print(packet)
		print(f'bytes_sent amount - {bytes_sent}')
...
def user_menu(conn, logger_adapter, connected_user, options):
	...
	try:
		...
		print(len(str(response_content)))
		print(f"Sending response with code - {response_code}")
...
def main_loop(options, log_file_name, users):
	...
	while True:
		print(WAIT_FOR_CONN)
		conn, addr = s.accept()
		print(NEW_CONN_ACCEPTED % str(addr))
